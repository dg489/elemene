	...
	<div class="shopcart">
		<div class="content" @click="toogleList">
			<div class="content-left">
				...
				<div class="price" :class="{'height-line':totalPrice>0}">{{totalPrice}}元</div>
				<div class="desc">另需配送费￥{{deliverPrice}}</div>
			</div>
			<div class="content-right" @click.stop="play">
				<div class="play" :class="playClass">
				{{playDesc}}
				</div>
			</div>
		</div>
...
//				 没值传过来的时候默认值
		...
		computed: {
			...
			listShow() {
				...
				if (show) {
					this.$nextTick(() => {
						if (!this.scroll) {
							this.scroll = new BetScoll(this.$refs.listCount, {
								click: true
							})
						} else {
							this.scroll.refresh()
		...
		methods: {
			drop(el) {
				let ballleng = this.balls.length
				for (let i = 0; i < ballleng; i++) {
			...
			play() {
				...
				window.alert(`请支付${this.totalPrice}`)
			}
		},
		components: {
			cartcontrol
